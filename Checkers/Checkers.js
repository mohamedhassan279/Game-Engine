...
export class Checkers extends GameEngine {
    ...
    check_for_jmp(i, j, currentTurn, board) {
        if (currentTurn == 1) {
            let right_child = [i + 1, j + 1],
                left_child = [i + 1, j - 1];
            if (this.is_within(right_child) && board[right_child[0]][right_child[1]] == 2) {
                //you have a valid left child and it's occupied with a checker of the other player
                let right_of_right_child = [i + 2, j + 2];
                if (this.is_within(right_of_right_child) && board[right_of_right_child[0]][right_of_right_child[1]] == 0) {
            ...
            if (this.is_within(left_child) && board[left_child[0]][left_child[1]] == 2) {
                //we do the same check for the left child
                let left_of_left_child = [i + 2, j - 2];
                if (this.is_within(left_of_left_child) && board[left_of_left_child[0]][left_of_left_child[1]] == 0) {
                    return "left jmp";
                }
            }
            return null;
            ...
            let right_child = [i - 1, j + 1],
                left_child = [i - 1, j - 1];
            if (this.is_within(right_child) && board[right_child[0]][right_child[1]] == 1) {
                //you have a valid left child and it's occupied with a checker of the other player
                let right_of_right_child = [i - 2, j + 2];
                if (this.is_within(right_of_right_child) && board[right_of_right_child[0]][right_of_right_child[1]] == 0) {
                    //if right right is empty then we need to move i,j to that empty spot (right right empty place)
            ...
            if (this.is_within(left_child) && board[left_child[0]][left_child[1]] == 1) {
                //we do the same check for the left child
                let left_of_left_child = [i - 2, j - 2];
                if (this.is_within(left_of_left_child) && board[left_of_left_child[0]][left_of_left_child[1]] == 0) {
                    return "left jmp";
    ...
    is_within(coord) {
        if (coord[0] >= this.ROWS || coord[0] < 0 || coord[1] >= this.COLS || coord[1] < 0) return false;
